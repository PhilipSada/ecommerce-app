...
class AuthController extends BaseController{
    ...
    {
        //this ensures that onces a user is registered and login they can't access the login and register pages
        if(isAuthenticated()){
            Redirect::to('/');
        }
    }
        ...
        if(Request::has('post')){
            ...
            if(CSRFToken::verifyCSRFToken($request->token)){
                $rules = [
                    ...
                    'fullname'=> ['required'=>true, 'minLength'=>6, 'maxLength'=> 50, 'unique'=>'users'],
    ...
    public function login(){
        if(Request::has('post')){
            ...
            if(CSRFToken::verifyCSRFToken($request->token)){
               ...
               if($user){
                   //to check if the password typed does not match the password in the database
